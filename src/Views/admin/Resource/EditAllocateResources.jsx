          ...
          <tbody>
            {recursosActuales.length > 0 ? (
              recursosActuales.map((r) => {
                ...
                // Solo deshabilitar si está en uso Y no fue preseleccionado
                const isDisabled = r.Status === "In_use" && !selected?.wasPreselected;
                ...
                return (
                  <tr key={r.ResourceId} className={isDisabled ? "disabled-row" : ""}>
                    ...
                    <td>
                      {r.ResourceName}
                      {selected?.wasPreselected && (
                        <small className="text-muted"> (ya asignado)</small>
                      )}
                    </td>
                    <td>{r.ResourceId}</td>
                    <td>
...
export default EditAssignResourcesModal;